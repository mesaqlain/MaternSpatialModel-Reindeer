...
## Script for parameter estimation of the JO region in the Reindeer data
...
# Design Matrices X
X <- model.matrix(model_frame, data=JO_data)
...
b0 <- c(-0.51, 0.01, 1.25, -2.21, -1.34, 0.09, -1.35, -0.26, 0.14)
length_b0 <- length(b0)
phi0 <- 1
s0 <- 1
r0 <- c(rep(0,ncol(Z)))
...
write.csv(data, 
          "JO_results2.csv", row.names = F, quote = F)
...
write.csv(data_2, 
          "JO_results_best2.csv", row.names = F, quote = F)